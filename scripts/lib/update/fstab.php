...
{
    if ($fstabPath === '' || pmssFilesystemPathHasNulByte($fstabPath)) { $logger('[WARN] Unsafe fstab path; skipping '.$context); return null; }
    if (!pmssRegularFilePathIsReadable($fstabPath)) { $logger('[WARN] '.$fstabPath.' is not a regular file; skipping '.$context); return null; }
