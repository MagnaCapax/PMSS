...
/**
    ...
    /**
     * Pin libssl3/openssl + the OpenSSH trio to the PECL-ssh2-compatible set via an
     * APT preferences file (Pin-Priority 1001). Debian 12 only.
     *
     * apt-mark hold only blocks automatic UPGRADES; it does NOT stop the resolver
     * from REMOVING openssh during dependency resolution (openssh deb12u9/u10 depend
     * on libssl3 >= 3.0.19, which conflicts with the held 3.0.17). The early
     * fix-broken / dpkg-selection phases in update-step2 therefore "resolve" the
     * broken state by removing the openssh trio — the cascade. Pin-Priority 1001
     * forces the compatible versions as candidates, so apt DOWNGRADES (never removes)
     * openssh, and the on-disk preferences file survives the package removal/reinstall
     * that clobbers apt-mark hold. Declarative cascade-PREVENTION written before the
     * package phase; the imperative convergence/heal functions above remain as RECOVERY
     * for already-damaged hosts and pin the same versions. Idempotent. Refs #436/#585.
     */
    function pmssWriteLibssl3OpensshAptPin(?int $distroVersion = null): void
    {
        $libsslTarget  = '3.0.17-1~deb12u2';
        $opensshTarget = '1:9.2p1-2+deb12u7';

        if (pmssEnvFlagEnabled('PMSS_DRY_RUN')) {
            logMessage('[DRY-RUN] pmssWriteLibssl3OpensshAptPin: skipping pin write');
            return;
        }

        $effectiveVersion = pmssDistroVersionFromEnv($distroVersion);
        if ($effectiveVersion !== 12) {
            logMessage('[SKIP] pmssWriteLibssl3OpensshAptPin: not Debian 12 (detected: '.$effectiveVersion.')');
            return;
        }

        $pinPath  = '/etc/apt/preferences.d/pmss-libssl3-openssh.pref';
        $contents = "# Managed by PMSS update-step2 (opensslSsh2Compat). Do not edit by hand.\n"
        ...
        // Prevention layer: a failed write degrades to the self-healing status quo
        // (the convergence/heal functions still run and keep SSH up), so do NOT throw
        // and abort the whole update — log and continue. Refs #436/#585.
        $tmpPath = $pinPath.'.pmss-tmp';
        if (@file_put_contents($tmpPath, $contents) === false || !@rename($tmpPath, $pinPath)) {
            @unlink($tmpPath);
            logMessage('[ERROR] pmssWriteLibssl3OpensshAptPin: failed to write '.$pinPath.' — continuing (convergence/heal still protects openssh)');
            return;
        }
        @chmod($pinPath, 0644);
