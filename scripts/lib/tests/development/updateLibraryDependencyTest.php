...
class UpdateLibraryDependencyTest extends TestCase
{
    ...
    public function testDirectLibraryConsumersKeepLeanIncludes(): void
    {
        $this->assertRepoFileDependencyCases([
            ...
            ['scripts/util/setupLetsEncrypt.php', ["pmssRequireRelativeFiles(__DIR__.'/../lib', ['update/distro.php', 'certbotSetup.php']);"], ["require_once __DIR__.'/../lib/update.php';"], 'setupLetsEncrypt.php should not pull scripts/lib/update.php just for distro detection'],
            ...
            ['scripts/cron/checkRootlessDocker.php', ["pmssRequireRelativeFiles('/scripts/lib', ['user/log.php', 'user/userConfigStore.php', 'user/watchdog.php']);", 'pmssUserDockerEnabled($user, $userConfigStore)'], ["function_exists('pmssUserDockerEnabled')"], 'checkRootlessDocker.php should rely on userConfigStore.php for Docker policy checks'],
            ['scripts/lib/motd/model.php', ["require_once __DIR__.'/../update/distro.php';", "require_once __DIR__.'/../version.php';"], ["require_once __DIR__.'/../update.php';"], 'Motd collector should not pull scripts/lib/update.php just for distro/version metadata'],
            ...
            ['scripts/lib/user/userConfigRuntime.php', ["pmssLogStatus('SKIP', 'Rootless Docker disabled by config for '.\$user['name']);", "pmssUserDockerEnabled(\$user['name'], \$store)"], ["function_exists('pmssLogStatus')", "function_exists('pmssUserDockerEnabled')"], 'userConfig runtime helpers should rely on loaded runtime/userConfigStore policy helpers'],
            ...
            ['scripts/lib/traffic/storage.php', ["'../user/identity.php'"], ["require_once __DIR__.'/../userLifecycle.php';"], 'traffic storage should only load username identity helpers'],
    ...
    public function testRuntimeBootstrapsKeepOnlyIntentionalFallbackGuards(): void
    {
        ...
        $this->assertRepoFileDependencyCases([
            ...
            ['scripts/lib/user/userConfigStore.php', ["require_once __DIR__.'/userConfigPolicy.php';", 'class UserConfigStore'], ["if (!function_exists('pmssUserDockerMinRamMiB')) {", "if (!function_exists('pmssUserConfigNormaliseToggleValue')) {", "if (!function_exists('pmssUserConfigResolvePayload')) {", "if (!function_exists('pmssUserDockerEnabled')) {", "if (!function_exists('pmssUserLighttpdEnabled')) {"], 'userConfigStore.php should load focused policy helpers directly once it is required'],
            ['scripts/lib/user/userConfigPolicy.php', ['function pmssUserDockerMinRamMiB(): int', 'function pmssUserConfigResolvePayload(string $username, ?UserConfigStore &$store = null): ?array', 'function pmssUserConfigFeatureEnabled(string $username, string $key, ?UserConfigStore $store = null): bool', 'function pmssUserDockerEnabled(string $username, ?UserConfigStore $store = null): bool', 'function pmssUserLighttpdEnabled(string $username, ?UserConfigStore $store = null): bool', 'function pmssUserPanelSessionLoginEnabled(string $username, ?UserConfigStore $store = null): bool'], ["if (!function_exists('pmssUserDockerMinRamMiB')) {", "if (!function_exists('pmssUserConfigNormaliseToggleValue')) {", "if (!function_exists('pmssUserConfigResolvePayload')) {", "if (!function_exists('pmssUserDockerEnabled')) {", "if (!function_exists('pmssUserLighttpdEnabled')) {", "if (!function_exists('pmssUserPanelSessionLoginEnabled')) {"], 'userConfigPolicy.php should define policy helpers without fallback guards'],
        ]);
    }
    ...
    public function testUserLifecycleBootstrapsUserLoggerForLeanCallers(): void
    {
        $this->assertRepoFileDependencyCases([
            ...
            ['scripts/lib/userLifecycle.php', ["'user/identity.php'", "'user/selection.php'", "'user/watchdog.php'"], ['function pmssUserWatchdogEnsureServices(', 'function pmssManagedUsersSelectFromCommand(', 'function pmssPasswdEntryLookup('], 'userLifecycle.php should compose focused user modules instead of owning their implementations'],
            ['scripts/lib/user/watchdog.php', ["pmssRequireRelativeFiles(__DIR__, ['log.php', 'selection.php', 'serviceLaunch.php']);", 'function pmssUserWatchdogEnsureServices('], ["require_once dirname(__DIR__).'/userLifecycle.php';", "if (!function_exists('pmssUserLogAllowed')) {"], 'user watchdog helpers should be a focused module without reloading the lifecycle facade'],
            ['scripts/cron/checkRtorrent.php', ["require_once __DIR__.'/../lib/rtorrent/watchdog.php';"], ["require_once __DIR__.'/../lib/user/log.php';", "function_exists('pmssUserLog')"], 'checkRtorrent.php should delegate rTorrent watchdog support to the focused helper'],
            ['scripts/lib/rtorrent/watchdog.php', ["require_once __DIR__.'/../user/log.php';", "pmssUserLog(\$user, 'checkRtorrent: '.\$message);"], ["function_exists('pmssUserLog')"], 'rTorrent watchdog helper should log through the required user logger directly'],
