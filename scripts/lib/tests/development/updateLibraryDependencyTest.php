...
class UpdateLibraryDependencyTest extends TestCase
{
    ...
    public function testUserLifecycleBootstrapsUserLoggerForLeanCallers(): void
    {
        foreach ([
            ...
            ['scripts/cron/checkDelugeInstances.php', ["require_once __DIR__.'/../lib/userLifecycle.php';", "pmssUserLog(\$thisUser, 'deluge stopped due to suspension');"], ['pmssUserLogPath', "\$canUserLog = function_exists('pmssUserLog');"], 'Deluge watchdog should use userLifecycle.php as its user logger bootstrap'],
            ['scripts/cron/checkLighttpdInstances.php', ["require_once __DIR__.'/../lib/userLifecycle.php';", "pmssUserLog(\$thisUser, 'lighttpd start requested');"], ['pmssUserLogPath', "\$canUserLog = function_exists('pmssUserLog');"], 'Lighttpd watchdog should use userLifecycle.php as its user logger bootstrap'],
            ['scripts/cron/checkQbittorrentInstances.php', ["require_once __DIR__.'/../lib/userLifecycle.php';", "pmssUserLog(\$thisUser, 'qbittorrent-nox start requested');"], ['pmssUserLogPath', "\$canUserLog = function_exists('pmssUserLog');"], 'qBittorrent watchdog should use userLifecycle.php as its user logger bootstrap'],
            ['scripts/cron/checkRcloneInstances.php', ["require_once __DIR__.'/../lib/userLifecycle.php';", "pmssUserLog(\$thisUser, 'rclone start requested');"], ['pmssUserLogPath', "\$canUserLog = function_exists('pmssUserLog');"], 'Rclone watchdog should use userLifecycle.php as its user logger bootstrap'],
