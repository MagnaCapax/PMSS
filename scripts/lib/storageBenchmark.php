 ...
 *
 * The util wrapper stays thin; this file owns argument parsing, benchmark
 * validation, JSONL entry construction, fio execution, and summaries.
 *
...
function storageBenchmarkEntryMetrics(array $entry): array
{
    return (isset($entry['metrics']) && is_array($entry['metrics'])) ? $entry['metrics'] : [];
}
...
function storageBenchmarkPreflightDisplay(array $entry, string $key): string
{
    ...
    $value = storageBenchmarkScalarDisplay($entry[$key]);
...
function storageBenchmarkEntryHasFileRw(array $entry): bool
{
    ...
    return storageBenchmarkScalarDisplay($entry['params']['rw'] ?? '') !== '';
}
...
{
    if (!array_key_exists('device', $entry)) {
        return null;
    }

    $device = storageBenchmarkScalarDisplay($entry['device']);
...
function storageBenchmarkShowLast(string $jsonLog): int
{
    ...
    foreach (pmssJsonLineFileRead($jsonLog) as $entry) {
        if (!isset($entry['run_id']) || !is_string($entry['run_id']) || $entry['run_id'] === '') {
            continue;
        }
    ...
    foreach ($run as $entry) {
        if (storageBenchmarkScalarDisplay($entry['test'] ?? '') === 'preflight-idle') {
    ...
    foreach ($run as $entry) {
        ...
        if ($test !== '' && storageBenchmarkEntryDeviceName($entry) === null && storageBenchmarkEntryHasFileRw($entry)) {
    ...
    $devices = [];
    foreach ($run as $entry) {
        $device = storageBenchmarkEntryDeviceName($entry);
        if ($device !== null) {
...
function storageBenchmarkRunDeviceTests(string $jsonLog, string $runTs, string $label, string $runId, int $ddSizeBytes, int $devRuntime): void
{
    ...
    foreach (pmssStorageHealthDiskInventoryRead() as $meta) {
        ...
        if ($size === null) { storageBenchmarkAppendJsonLine($jsonLog, storageBenchmarkDeviceEntryBuild($base, $meta, $path, 'device-preflight', ['ok' => false, 'error' => 'unable to determine block device size'])); printf("%s\tskipped: unable to determine block device size\n", $path); continue; }
