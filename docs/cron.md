...
- `cgroupBfqWeightApply.php` - Reapply per-user cgroup kernel I/O weights
  directly from PMSS user configuration. It writes cgroup-v1 BFQ weights on
  legacy hosts and cgroup-v2 `io.bfq.weight` or `io.weight` on unified hosts so
  bonus-aware IOWeight tiers stay consistent.
