...
| `servarr.php` | Retained as the shared ARR updater entrypoint, but excluded from the default update-step2 app autoloader. | Per-account media-stack installs are handled from the skeleton/user tooling. `arr.php` installs but NEVER executes an ARR binary: version detection reads `version.txt`/`VERSION` only, because these applications ignore an unrecognised `--version` and boot as a root daemon instead (ADR 0034). |
