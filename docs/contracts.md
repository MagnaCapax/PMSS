...
- arrRootGuard.php (`scripts/lib/`)
  - `PMSS_ROOT_GUARD_APP_INSTALL_ROOTS` is the one catalog for known consumer executable roots (the ARR family, qBittorrent, Deluge, Transmission, Jellyfin, SABnzbd, rTorrent, and Syncthing). `pmssArrRootGuardAuditAndKill()` selects real uid 0 from `/proc` by executable path, kills only catalogued applications, and signals the process group when the pid leads it (ADR 0035). A uid-0 executable outside standard system paths is reported as alert-only; customer-uid processes and look-alike paths are not selected. Selection is exe-path + uid, never a cmdline match: customers run their own copies from `/home/<user>/.bin/<App>` and a cmdline match would kill them. Called from `scripts/cron/mediaStackInstancesCheck.php` (root, every 2 minutes); findings emit `###PMSS_ROOT_GUARD_ALERT` and make the cron exit non-zero, while a clean run is silent.
